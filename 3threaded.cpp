...
{
	std::ofstream fout("3threadedMandelbrot"+std::to_string(i)+".ppm");
	fout << "P3" << std::endl;
...
void generateImage()
{
    ...
    std::thread thread1(generateImageSection, std::ref(colorVect), 0, imageHeight/3);
    std::thread thread2(generateImageSection, std::ref(colorVect), imageHeight/3, (imageHeight/3)*2);
    std::thread thread3(generateImageSection, std::ref(colorVect), (imageHeight/3)*2, imageHeight);
