...
{
	std::ofstream fout("2threadedMandelbrot"+std::to_string(i)+".ppm");
	fout << "P3" << std::endl;
...
void generateImage()
{
    ...
    std::thread thread1(generateImageSection, std::ref(colorVect), 0, imageHeight/2);
    std::thread thread2(generateImageSection, std::ref(colorVect), imageHeight/2, imageHeight);
